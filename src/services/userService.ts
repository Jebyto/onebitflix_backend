...
function filterLastEpisodesByCourse(episodes: EpisodeInstance[]) {
    ...
    const lastEpisodes: EpisodeInstance[] = episodes.reduce((currentList, episode) => {
    ...
    getKeepWatchingList: async (id: number) => {
        const userWithWatchingEpisodes = await User.findByPk(id, {
            include: {
                ...
                attributes: ['id', 'name', 'order', 'synopsis', 
                    ['course_id', 'courseId'],
                    ['video_url', 'videoUrl'], 
        ...
        //@ts-ignore
        keepWatchingList.sort((a, b) => a.watchTime!.updatedAt < b.watchTime!.updatedAt
            ? 1 : -1);
